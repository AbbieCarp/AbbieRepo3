...
### 1.2.1 Task Property Validation Rules

- `id`
  - Type must be `string`.
  - Must be a non-empty UUID string generated by `crypto.randomUUID()`.
...
- `title`
  ...
  - Reject titles that become empty after trimming.

- `description`
  - Type must be `string` when provided.
  - Defaults to `''` when omitted.
  - Max length is `500` characters.
  - Stored as a trimmed string.
...
- `createdAt`
  ...
  - Set once at creation and never changed by update operations.

- `updatedAt`
  - Type must be `string`.
  - Must be a valid ISO 8601 UTC timestamp.
  - Must be greater than or equal to `createdAt`.
  - Refreshed only after a successful create/update mutation.
